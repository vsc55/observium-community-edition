...
$navbar['options_right']['add']['userlevel'] = 9;
...
foreach (array_keys($definitions['transports']) as $transport) {
    $form_params['method'][$transport] = $definitions['transports'][$transport]['name'];

    if (isset($definitions['transports'][$transport]['docs'])) {
        // Known key in docs page (use if transport name is different with docs page)
        $docs_link = OBSERVIUM_DOCS_URL . '/alerting_transports/#' . $definitions['transports'][$transport]['docs'];
    } else {
        $docs_link = OBSERVIUM_DOCS_URL . '/alerting_transports/#' . str_replace(' ', '-', strtolower($definitions['transports'][$transport]['name']));
    }
...
foreach ($definitions['transports'] as $transport => $data) {
    $row++;
...
foreach (array_keys($definitions['transports']) as $transport) {
    ...
    foreach (array_keys($definitions['transports']) as $ltransport) {
