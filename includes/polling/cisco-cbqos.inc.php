...
foreach ($cm_stats as $policy_index => $policy_entry) {
    $port = get_port_by_index_cache($device['device_id'], $service_policies[$policy_index]['cbQosIfIndex']);
    ...
    foreach ($policy_entry as $object_index => $object_entry) {
        ...
        if (isset($cbq_table[$policy_index][$object_index])) {
            ...
            foreach ($metrics as $oid => $value) {
                $diff = int_sub($value, $db_object[$oid]); // Use accurate substract
            ...
            $db_update                = array_merge($metrics, $metrics_computed);
            ...
            $db_update['cbqos_lastpolled'] = $polled;
            ...
            $db_insert = [
                ...
                'direction'    => $object_entry['direction'],
                'cbqos_lastpolled' => $polled,
            ];
