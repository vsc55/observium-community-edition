 ...
 * @return bool
 */
function resizeImage($image, $targetImage=NULL, $maxWidth=0, $maxHeight=0)
{
...
function wm_debug($string)
{
	...
	if ($weathermap_debugging)
	{
		...
		{ print_debug("DEBUG:$calling_fn " . ($weathermap_map==''?'':$weathermap_map.": ") . rtrim($string), true, "WEATHERMAP"); }
		else
		{
			...
			if(TRUE)
			{
				$log=fopen('/tmp/weathermap-debug.log', 'a');
...
function unwm_format_number($instring, $kilo = 1000)
{
	$matches=0;
	$number=0;
	...
	if (preg_match("/([0-9\.]+)(M|G|K|T|m|u)/", $instring, $matches))
	{
		...
		if ($matches[2] == 'm') { $number /= $kilo; }
		if ($matches[2] == 'u') { $number /= $kilo * $kilo; }
	}
