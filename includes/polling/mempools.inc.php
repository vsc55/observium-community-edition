...
foreach (dbFetchRows('SELECT * FROM `mempools` WHERE `device_id` = ?', [ $device['device_id'] ]) as $mempool) {
    ...
    if (is_file($file)) {
        ...
        $mempool = array_merge($mempool, calculate_mempool_properties($mempool['mempool_multiplier'],
                                                                      $mempool['used'],
                                                                      $mempool['total'],
                                                                      $mempool['free'],
                                                                      $mempool['perc']));
    } elseif (!safe_empty($mempool['mempool_object'])) {
        // Check if we can poll the device ourselves with generic code using definitions.
        // Object is always set when definitions add mempools.
        $table_def = mempool_find_definition_by_oid($mempool['mempool_mib'], $mempool['mempool_object']);
    ...
    $rrd_filename = get_mempool_rrd($device, $mempool, FALSE);
    rename_rrd($device, 'mempool-' . $mib_lower . '-' . $mempool['mempool_index'], 'mempool-' . $rrd_filename);

    rrdtool_update_ng($device, 'mempool', [ 'used' => $mempool['used'], 'free' => $mempool['free'] ], $rrd_filename);
