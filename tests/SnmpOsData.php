...
class SnmpOsData extends \PHPUnit\Framework\TestCase {
    ...
    public static function providerGetDeviceOS() {
        ...
        if ($handle = opendir($snmpsimd_data)) {
            while (false !== ($file = readdir($handle))) {
                if (filetype($snmpsimd_data . '/' . $file) === 'file' &&
                    preg_match('/^(?<community>(?<os>[\S]+?)(?:\-\d+)?)\.snmprec$/', $file, $matches)) {

                    //if (!str_starts($matches['os'], 'aten')) { continue; }
  ...
  public static function providerGetDeviceOS2()
  {
    return array(
  ...
  */
  ...
  public static function providerIsSNMPable()
  {
    return array(
