...
class SnmpData extends \PHPUnit\Framework\TestCase {
  ...
  */
  ...
  public static function providerSnmp_Get()
  {
  ...
  */
  ...
  public static function providerSnmpGetOid()
  {
    $community_ftos     = 'ftos-9.10';
  ...
  */
  ...
  public static function providerSnmp_UTF8()
  {
...
32768K bytes of memory,3584K bytes of flash';
  ...
  */
  ...
  public static function providerSnmpGetMultiOid()
  {
    $community_ftos     = 'ftos-9.10';
    $community_ubuntu   = 'ubuntu-16.04';
  ...
  public function testSnmpWalkBareOid($community, $flags, $oids, $mib, $result_file)
  {
    ...
    $result = json_decode(file_get_contents(__DIR__ . '/data/snmp/' . $result_file), TRUE);
    $this->assertSame($result, $test);
  }

  public static function providerSnmpWalkBareOid()
  {
  ...
  */
  ...
  public static function providerSnmpWalkMultiPartOid()
  {
    $community_ftos     = 'ftos-9.10';
    $community_ubuntu   = 'ubuntu-16.04';
  ...
  */
  ...
  public static function providerSnmpWalkThreepartOid()
  {
    ...
    /**
     * @dataProvider providerSnmpGetCache
     * @group snmpcache
     */
    public function testSnmpCacheOid($oid, $mib, $result, $error = OBS_SNMP_ERROR_OK) {
        global $snmpsimd_ip, $snmpsimd_port;

        $device = build_initial_device_array($snmpsimd_ip, 'ubuntu-16.04', 'v2c', $snmpsimd_port, 'udp');
        $device['snmp_timeout'] = 2;
        $device['snmp_retries'] = 1;
        //var_dump($device);
        ...
        // first request for caching
        $test1 = snmp_cache_oid($device, $oid, $mib);
        $this->assertSame($result, $test1);
        $this->assertSame($error, snmp_error_code());
        $this->assertTrue(mem_cache_exists($cache_key));

        // second request must be same as first and cached!
        $test2 = snmp_cache_oid($device, $oid, $mib);
        $this->assertSame($result, $test2);
        $this->assertSame(OBS_SNMP_ERROR_CACHED, snmp_error_code());

        // Stats
        $stat = mem_cache_stat();
        //var_dump($stat);
        $this->assertContains($cache_key, $stat['items'], "SNMP cache doesn't contains $cache_key");
        $this->assertGreaterThanOrEqual(1, $stat['count']);
    }

    public static function providerSnmpGetCache() {

        return [
            [ 'sysDescr.0',             'SNMPv2-MIB', 'Linux dev 4.4.0-77-generic #98-Ubuntu SMP Wed Apr 26 08:34:02 UTC 2017 x86_64' ],
            [ 'SNMPv2-MIB::sysDescr.0', '',           'Linux dev 4.4.0-77-generic #98-Ubuntu SMP Wed Apr 26 08:34:02 UTC 2017 x86_64' ],
            [ '.1.3.6.1.2.1.1.1.0',     '',           'Linux dev 4.4.0-77-generic #98-Ubuntu SMP Wed Apr 26 08:34:02 UTC 2017 x86_64' ],
            // Not exist OID or error
            [ '.1.3.6.1.2.1.1.1.0.0.0', '',           '', OBS_SNMP_ERROR_FAILED_RESPONSE ],
        ];
    }

    /**
     * @dataProvider providerSnmpTableCache
     * @group snmpcache
     */
    public function testSnmpCacheTable($oid, $mib, $result)
    {
        global $snmpsimd_ip, $snmpsimd_port;
        ...
        // first request for caching
        $test1 = snmp_cache_table($device, $oid, [], $mib);
        $this->assertSame($result, $test1);
        $this->assertSame(OBS_SNMP_ERROR_OK, snmp_error_code());
        $this->assertTrue(mem_cache_exists($cache_key));

        // second request must be same as first and cached!
        $test2 = snmp_cache_table($device, $oid, [], $mib);
        $this->assertSame($result, $test2);
        $this->assertSame(OBS_SNMP_ERROR_CACHED, snmp_error_code());
    ...
    public static function providerSnmpTableCache() {

        $arr = [
            196608 => [ 'hrDeviceDescr' => 'GenuineIntel: Common KVM processor' ],
            196609 => [ 'hrDeviceDescr' => 'GenuineIntel: Common KVM processor' ],
            196610 => [ 'hrDeviceDescr' => 'GenuineIntel: Common KVM processor' ],
            196611 => [ 'hrDeviceDescr' => 'GenuineIntel: Common KVM processor' ],
            262145 => [ 'hrDeviceDescr' => 'network interface lo' ],
            262146 => [ 'hrDeviceDescr' => 'network interface eth0' ],
            786432 => [ 'hrDeviceDescr' => 'Guessing that there\'s a floating point co-processor' ]
        ];

        $arr2 = [
            '2.77.222.50.17' => [ 'ipNetToMediaPhysAddress' => '0:0:c:9f:f1:f8' ],
            '2.77.222.50.18' => [ 'ipNetToMediaPhysAddress' => '7c:ad:74:d:9c:ff' ],
            '2.77.222.50.26' => [ 'ipNetToMediaPhysAddress' => 'f2:a3:52:32:c7:e' ],
            '2.77.222.50.29' => [ 'ipNetToMediaPhysAddress' => '36:31:35:34:31:30' ]
        ];

        return [
            [ 'hrDeviceDescr',                     'HOST-RESOURCES-MIB', $arr ],
            [ 'HOST-RESOURCES-MIB::hrDeviceDescr', '',                   $arr ],
            // multipart index
            [ 'IP-MIB::ipNetToMediaPhysAddress',   '',                   $arr2 ],
        ];
    }

    /**
     * @dataProvider providerSnmpTableCache2
     * @group snmpcache
     */
    public function testSnmpCacheTable2($oid, $mib, $result) {
        global $snmpsimd_ip, $snmpsimd_port;
        ...
        $cache_key = snmp_cache_key($device, $mib, $oid, 'twopart');

        // first request for caching
        $test1 = snmp_cache_table_twopart($device, $oid, [], $mib);
        $this->assertSame($result, $test1);
        $this->assertSame(OBS_SNMP_ERROR_OK, snmp_error_code());
        $this->assertTrue(mem_cache_exists($cache_key));

        // second request must be same as first and cached!
        $test2 = snmp_cache_table_twopart($device, $oid, [], $mib);
        $this->assertSame($result, $test2);
        $this->assertSame(OBS_SNMP_ERROR_CACHED, snmp_error_code());

        // Additional test, not exist cache key for single part oid
        $this->assertFalse(mem_cache_exists(snmp_cache_key($device, $mib, $oid)));
    }

    public static function providerSnmpTableCache2() {

        $arr2 = [
            2 => [
                '77.222.50.17' => [ 'ipNetToMediaPhysAddress' => '0:0:c:9f:f1:f8' ],
                '77.222.50.18' => [ 'ipNetToMediaPhysAddress' => '7c:ad:74:d:9c:ff' ],
                '77.222.50.26' => [ 'ipNetToMediaPhysAddress' => 'f2:a3:52:32:c7:e' ],
                '77.222.50.29' => [ 'ipNetToMediaPhysAddress' => '36:31:35:34:31:30' ]
            ]
        ];
    ...
    /**
     * @dataProvider providerSnmpTableCache3
     * @group snmpcache
     */
    public function testSnmpCacheTable3($oid, $oid2, $result) {
        global $snmpsimd_ip, $snmpsimd_port;
        ...
        $cache_key = snmp_cache_key($device, NULL, $oid);
        $cache_key2 = snmp_cache_key($device, NULL, $oid2);

        // first request for caching
        $test1 = snmp_cache_table($device, $oid, []);
        $test1 = snmp_cache_table($device, $oid2, $test1);
        $this->assertSame($result, $test1);
        $this->assertSame(OBS_SNMP_ERROR_OK, snmp_error_code());
        $this->assertTrue(mem_cache_exists($cache_key));
        $this->assertTrue(mem_cache_exists($cache_key2));

        // second request must be same as first and cached!
        $test2 = snmp_cache_table($device, $oid, []);
        $test2 = snmp_cache_table($device, $oid2, $test2);
        $this->assertSame($result, $test2);
        $this->assertSame(OBS_SNMP_ERROR_CACHED, snmp_error_code());
    }

    public static function providerSnmpTableCache3() {

        $arr2 = [
            '2.77.222.50.17' => [ 'ipNetToMediaNetAddress' => '77.222.50.17', 'ipNetToMediaPhysAddress' => '0:0:c:9f:f1:f8' ],
            '2.77.222.50.18' => [ 'ipNetToMediaNetAddress' => '77.222.50.18', 'ipNetToMediaPhysAddress' => '7c:ad:74:d:9c:ff' ],
            '2.77.222.50.26' => [ 'ipNetToMediaNetAddress' => '77.222.50.26', 'ipNetToMediaPhysAddress' => 'f2:a3:52:32:c7:e' ],
            '2.77.222.50.29' => [ 'ipNetToMediaNetAddress' => '77.222.50.29', 'ipNetToMediaPhysAddress' => '36:31:35:34:31:30' ]
        ];

        return [
            // multipart index
            [ 'IP-MIB::ipNetToMediaPhysAddress',   'IP-MIB::ipNetToMediaNetAddress', $arr2 ],
        ];
    }
}
