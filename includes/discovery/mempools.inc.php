...
foreach (get_device_mibs_permitted($device) as $mib) {
    ...
    foreach ($config['mibs'][$mib]['mempool'] as $entry_name => $entry) {
        ...
        // Determine identifier for mempool_object using 3-tier priority
        $object_identifier = entity_object_definition($entry, $entry_name, 'mempool');

        // Skip broken definitions - this fallback should never be used
        if (safe_empty($object_identifier)) {
            echo "]";
            print_debug("No valid identifier found for mempool entry $entry_name in $mib - skipping");
            continue;
        }
        ...
        if ($entry['type'] === 'table' || !isset($entry['type'])) {
            ...
            foreach ($mempools_array as $index => $mempool_entry) {
                ...
                if (is_numeric($mempool['used']) && is_numeric($mempool['total'])) {
                    ...
                    discover_mempool($valid['mempool'], $device, $index, $mib, $descr, $scale, $mempool['total'], $mempool['used'], $mempool_hc, [ 'table' => $object_identifier]);
                    $entry['found'] = TRUE;
                }
                $i++;
            }
