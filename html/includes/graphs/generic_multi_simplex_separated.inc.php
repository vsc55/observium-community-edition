...
foreach ($rrd_list as $i => $rrd) {
    ...
    if ($rrd['colour']) {
        $colour = $rrd['colour'];
    } elseif (isset($scheme_colours[$i])) {
        $colour = ltrim($scheme_colours[$i], '#');
    } else {
    ...
    if ($rrd['invert']) {
        ...
        if (!$nototal) {
            $rrd_optionsc .= " GPRINT:tot" . $rrd['ds'] . $i . ":%5.2lf%s" . rrdtool_escape($total_units);
        }
        $rrd_optionsc .= "'\\n' COMMENT:'\\n'";
    } else {
        ...
        if (!$nototal) {
            $rrd_optionsb .= " GPRINT:tot" . $rrd['ds'] . $i . ":%5.2lf%s" . rrdtool_escape($total_units);
        }
        $rrd_optionsb .= "'\\n' COMMENT:'\\n'";
    }
}
