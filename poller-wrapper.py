...
"""
 ...
 @package    observium
 @subpackage poller
 @copyright  (C) 2013-2014 Job Snijders, (C) 2014-2025 Adam Armstrong
"""
...
if python_version < (3, 0):
    print("ERROR: Python 2.x is not supported")
    print("Install python3 for your system")
    sys.exit(2)

# Python 3.x specific modules
import queue

# Database driver: use PyMySQL only
try:
    import pymysql

    db_version = "pymysql " + pymysql.__version__
except ImportError:
...
def printworker():
    """
        A separate queue and a single worker for printing information to the screen prevents
        the good old joke:
...
def process_worker():
    """
        This function will fork off single instances of the poller.php process, record
    ...
    while True:
        ...
        try:
            ...
            print("ERROR: worker %s failed for %s %s: %s" % (threading.current_thread().name, entity, entity_id, e), flush=True)
        finally:
    ...
    if process == 'discovery':
        ...
        for mib_index in mib_indexes:
            ...
            try:
                ...
                print("         see: https://docs.observium.org/faq/"
...
devices_list = []
exit_code = 0  # Default exit code, updated based on execution result
process_id = None  # Database process id for observium_processes tracking
...
db_params = {
    ...
    'port': db_port,
    'charset': 'utf8mb4',
    'autocommit': True,  # enable autocommit for pymysql lib
}
if bool(db_socket):
...
# ******************************
# MAIN WRAPPER LOGIC STARTS HERE
# ******************************
try:
    """ 
...
# =======================
# ==== MAIN WRAPPER LOGIC ENDS HERE ====
# =======================
finally:
    print("INFO: finalising and cleanup wrapper")
    # Remove process info from DB if it was created
    if process_id is not None:
        try:
            p_query = """DELETE FROM `observium_processes` WHERE `process_id` = %s"""
            cursor.execute(p_query, (process_id,))
            # db.commit()  # Autocommit is enabled; this is kept for clarity
        except Exception:
    ...
    try:
        ...
        # Ignore errors when closing the cursor
        pass
    try:
        db.close()
    except Exception:
        # Ignore errors when closing the database connection
        pass

    # Close null file handle if it was opened
    if 'FNULL' in globals():
        try:
            FNULL.close()
        except Exception:
            # Ignore errors when closing the null file handle
            pass
