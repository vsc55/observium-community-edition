...
// NOTES. phpfastcache 8.x supports php 7.3 - 8.x
 ...
 *
 * @param string  $key    Identification name
 * @param boolean $global Set to TRUE for do not prepend keys
 *
 * @return string Prepended key identification (ie 'data' -> 'mysql|user|10|data')
 */
...
/**
 * Call to getItem() method to retrieve a cache Item object.
 *
 * @param string $key Item identifier; for WUI the key is auto-prepended
 *                    with user identifier via get_cache_key().
 *
 * @return object|null Returns cache item object or null on failure.
 */
function get_cache_item($key) {
    // Ensure cache link is defined and is a valid object
    if (!defined('OBS_CACHE_LINK') ||
        !isset($GLOBALS[OBS_CACHE_LINK]) ||
        !is_object($GLOBALS[OBS_CACHE_LINK])) {

        // Cache is not enabled or improperly initialised
        return NULL;
    }
    ...
    try {
        // Attempt to fetch item from the cache storage
        return $observium_cache->getItem($observium_cache_key);
    } catch (\Phpfastcache\Exceptions\PhpfastcacheInvalidArgumentException $e) {

        // This usually indicates an invalid key or corrupted internal state
        $observium_cache->clear();
        if (OBS_DEBUG || OBS_CACHE_DEBUG) {
            print_warning('<span class="text-warning">CLEAR BROKEN CACHE</span>');
        }

        // Log detailed exception message for later analysis
        if ($GLOBALS['config']['devel']) {
            logfile('debug.log', 'CLEAR BROKEN CACHE (InvalidArgument): ' . $e->getMessage() .
                                 ' [key=' . $observium_cache_key . ']');
        }
    } catch (\Phpfastcache\Exceptions\PhpfastcacheDriverException $e) {

        // Driver-level problem (backend unavailable, connection lost, etc.)
        $observium_cache->clear();

        if (OBS_DEBUG || OBS_CACHE_DEBUG) {
            print_warning('<span class="text-warning">CLEAR BROKEN CACHE (DriverException)</span>');
        }

        if ($GLOBALS['config']['devel']) {
            logfile('debug.log', 'CLEAR BROKEN CACHE (DriverException): ' . $e->getMessage() .
                                 ' [key=' . $observium_cache_key . ']');
        }
    }

    // On any error path, return null
    return NULL;
}
...
function get_cache_session($key) {
    ...
    if (defined('OBS_CACHE_LINK') && $GLOBALS[OBS_CACHE_LINK] === (object)$GLOBALS[OBS_CACHE_LINK]) {
